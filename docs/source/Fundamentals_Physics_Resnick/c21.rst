...
Identificar los electrones de conducción y explicar su función para crear un objeto conductor con carga negativa o positiva.
...
Explique cómo un objeto cargado puede generar carga inducida en un segundo objeto.
...
apply Coulomb’s law to relate the magnitude of the electrostatic force, the charge magnitudes of the particles, and the
separation between the particles.

Para cualquiera de las partículas de un par de partículas cargadas, aplique la ley de Coulomb para relacionar la magnitud de la fuerza electrostática, las magnitudes de carga de las partículas y la separación entre las partículas.

21.10 Identify that Coulomb’s law applies only to (point-like)
particles and objects that can be treated as particles.

Identificar que la ley de Coulomb se aplica únicamente a partículas (puntuales) y objetos que pueden tratarse como partículas.

21.11 If more than one force acts on a particle, find the net
force by adding all the forces as vectors, not scalars.

Si más de una fuerza actúa sobre una partícula, encuentre la fuerza neta sumando todas las fuerzas como vectores, no escalares.
...
where :math:`\epsilon_0 = 8.85 \times 10^{-12} C^2/N \cdot m^2` is the permittivity constant. The ratio :math:`1/4 \pi \epsilon_0` is often replaced with the electrostatic
...
plates and C is an electrically neutral copper
...
in Fig. 21-6, where particle 1 has charge q1 and particle 2 has charge q2. (These symbols can represent either positive or negative charge.) Let’s also focus on particle 1
...
:math:`1/4 \pi \epsilon_0`. Then the magnitude of the electrostatic force in Coulomb’s law becomes
...
.. math::
   ...
   k = \frac{1}{4 \pi \epsilon_0} = 8.99 \times 10^9 N \times m^2/C^2
...
.. math::
   ...
   \overrightarrow{F}_{1,net} = \overrightarrow{F}_{12} + \overrightarrow{F}_{12} +\overrightarrow{F}_{13} +\overrightarrow{F}_{14} + ... + \overrightarrow{F}_{1n}
...
What are the magnitude and direction of the electrostatic force :math:`\overrightarrow{F}_{12}`
...
has charge :math:`q_3 = -3.20 \times 10^{-19} C` and is at a distance :math:`\frac{3}{4}R` from
article 1.What is the net electrostatic force :math:`\overrightarrow{F}_{1,net}` on particle 1
...
Three particles: To find the magnitude of :math:`\overrightarrow{F}_{13}`, we can
rewrite Eq. 21-4 as
...
.. math::
   ...
   \overrightarrow{F} = (2.05 \times 10^{-24} N) \hat{i}
